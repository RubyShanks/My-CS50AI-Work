...
class Sentence():
    ...
    def known_safes(self):
        ...
        """
        if (self.count == 0 and self.cells != set()):
            return self.cells
        else:
            return set()
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        if cell[0] > 0:
            tmpcell = (cell[0] - 1, cell[1])
            if tmpcell in self.mines:
                count -= 1
            if tmpcell not in self.safes and tmpcell not in self.mines:
                neighbors.add(tmpcell)
            if cell[1] > 0:
                tmpcell = (cell[0] - 1, cell[1] - 1)
                if tmpcell in self.mines:
                    count -= 1
                if tmpcell not in self.safes and tmpcell not in self.mines:
                    neighbors.add(tmpcell)
        ...
        while changed:
            ...
            for asentence in self.knowledge:
                tmpmines = tmpmines.union(asentence.known_mines())
                tmpsafes = tmpsafes.union(asentence.known_safes())
            if (tmpmines != set() or tmpsafes != set()):
                changed = True
            for tmpmine in tmpmines:
                self.mark_mine(tmpmine)
            for tmpsafe in tmpsafes:
                self.mark_safe(tmpsafe)             
